                        ...
                        foreach(var member in notification.ResourceData?.Members)
                        {
                            ...
                            if(user != null)
                            {
                                ...
                                if(changeType == "added")
                                {
                                    ...
                                    var createdUser = await _swaggerHubUserManagementService.CreateMember(
                                ...
                                else if(changeType == "deleted")
                                {
                                    ...
                                    var deletedUser = await _swaggerHubUserManagementService.DeleteMember(
                                        _groupConfiguration.ActiveDirectoryGroups.FirstOrDefault(g => g.ObjectId == notification.ResourceData?.Id)
                                        ,user.Mail
                                    );
                                ...
                                //Test Get from SwaggerHub
                                //var swaggerHubUser = await _swaggerHubUserManagementService.GetMemberByEmail(_groupConfiguration.ActiveDirectoryGroups.FirstOrDefault(g => g.ObjectId == notification.ResourceData?.Id), user.Mail);
                                //logger.LogInformation($"Username: {swaggerHubUser.UserName}, FirstName: {swaggerHubUser.FirstName}, LastName: {swaggerHubUser.LastName}, Email: {swaggerHubUser.Email}, Role: {swaggerHubUser.Role}");
                            }
                        }
                    }                    
